...
(function () {
  ...
  /** @ngInject */
  function FailureTypeController($mdSidenav, $scope, $state, homeApi, $timeout, accountApi, masterDataApi) {
    var vm = this;
    $scope.actionName = 'Create';
    $scope.pageSize = 10;
    $scope.tabName = 'Failure Type';
    ...
    $scope.dataSource = new kendo.data.HierarchicalDataSource({
      transport: {
        read: function (options) {
          return masterDataApi.getFaliureTypeTreeSource(options);
        }
      },
      schema: {
        model: {
    ...
    $scope.doSubmit = function () {
      ...
      if (validator.validate()) {
        if ($scope.actionName === "Create") {
          masterDataApi.saveFailureType("", $scope.newWorkType,$scope.descripion, "Create").then(function (data) {
          ...
          masterDataApi.saveFailureType($scope.dataItem.id, $scope.newWorkType, $scope.descripion, "Edit").then(function (data) {
            if (data) {
              $scope.treeViewOptions.dataSource.read();
              $('#dataPopup').modal('hide');
    ...
    $("#tree-context-menu").kendoContextMenu({
      ...
      select: function (e) {
        ...
        var data = treeView.dataItem(node);
        debugger;
        $scope.dataItem = _.pick(data, ['id', 'uid', 'name','descripion','type']);
        ...
        switch (action) {
          ...
          case "Edit":
            ...
            $scope.descripion = $scope.dataItem.descripion;;
            $scope.$digest();
            $('#dataPopup').modal('show');
            break;
          case "Delete":
            ...
            if (window.confirm("Would you like to delete this item?")) {
              masterDataApi.saveFailureType($scope.dataItem.id,"", "", "Delete").then(function (data) {
                if (data) {
                  $scope.treeViewOptions.dataSource.read();
                  $('#dataPopup').modal('hide');
