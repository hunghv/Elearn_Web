(function () {
  ...
  /** @ngInject */
  function WorkTypeController($mdSidenav, $scope, $state, homeApi, $timeout, accountApi, masterDataApi) {
    var vm = this;
    $scope.actionName = 'Create';
    $scope.pageSize = 10;
    $scope.tabName = 'Work Type';
    ...
    $scope.dataSource = new kendo.data.HierarchicalDataSource({
      transport: {
        read: function(options) {
          return masterDataApi.getWorkTypeTreeSource(options);
        }
      },
      schema: {
        model: {
    ...
    $scope.doSubmit = function () {
      ...
      if (validator.validate()) {
        if ($scope.actionName === "Create") {
          masterDataApi.saveWorkType("", $scope.newWorkType, "Create").then(function (data) {
          ...
          masterDataApi.saveWorkType($scope.dataItem.id, $scope.newWorkType, "Edit").then(function (data) {
            if (data) {
              $scope.treeViewOptions.dataSource.read();
              $('#dataPopup').modal('hide');
    ...
    $("#tree-context-menu").kendoContextMenu({
      ...
      select: function (e) {
        ...
        $scope.dataItem = _.pick(data, ['id', 'uid', 'name']);
        ...
        switch (action) {
          ...
          if (window.confirm("Would you like to delete this item?")) {
            masterDataApi.saveWorkType($scope.dataItem.id,"", "Delete").then(function (data) {
              if (data) {
                $scope.treeViewOptions.dataSource.read();
                $('#dataPopup').modal('hide');
